    ...
    on_supported_os.each do |os, os_facts|
      context "with #{os}" do
        ...
        context 'with valid options' do
          ...
          it {
            expect(subject).to create_file("/usr/share/puppet_ds389_config/ds_#{title}_setup.inf")
              .with_owner('root')
              .with_group('root')
              .with_mode('0600')
          ...
          it {
            expect(subject).to create_exec("Setup #{title} DS")
              .with_command("/sbin/setup-ds.pl --silent -f /usr/share/puppet_ds389_config/ds_#{title}_setup.inf")
              .with_creates("/etc/dirsrv/slapd-#{title}/dse.ldif")
              .that_requires("File[/usr/share/puppet_ds389_config/ds_#{title}_setup.inf]")
          ...
          context 'with conflicting resource port' do
            ...
            it {
              pending('https://github.com/puppetlabs/puppetlabs-stdlib/pull/1115')
